...
alias git-switch-to-gh-ssh='git remote set-url origin "$(git remote get-url origin | sed "s|^https://github.com/|git@github.com:|")"'
...
alias vpn-canonical='(cd ~/.sesame/; sudo openvpn --mute-replay-warnings --config ~/.sesame/us-nobuto.conf)'
alias vpn-canonical-dns='(cd ~/.sesame/; sed -e "s|/scripts/|/|" /usr/share/doc/openvpn-systemd-resolved/update-systemd-resolved.conf | sudo openvpn --mute-replay-warnings --config ~/.sesame/us-nobuto.conf --config /dev/stdin)'
